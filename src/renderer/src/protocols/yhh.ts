...
export const yhhProtocol: DeviceProtocol = {
  ...
  commands: {
    ...
    getBattery: [
      0x55, 0x30, 0xA5, 0x0B, 0x2E, 0x01, 0x01, 0x00,
      ...new Array(56).fill(0)
    ],
    ...
    getReportRate: [
      0x55, 0xED, 0x00, 0x01, 0x2E, 0x00, 0x00, 0x00,
      ...new Array(56).fill(0)
    ],
    ...
    getButtonMapping: [
      0x55, 0x08, 0xA5, 0x0B, 0x20, 0x00, 0x00, 0x00,
      ...new Array(56).fill(0)
    ],
  ...
  parsers: {
    ...
    deviceInfo: (response: Uint8Array) => {
      ...
      while (nameEnd < response.length && response[nameEnd] !== 0xAA && response[nameEnd] !== 0x00) {
        nameEnd++
      }
      const name = String.fromCharCode(...Array.from(response.slice(dataStart, nameEnd))) || 'YJX Mouse'

      // 查找版本号 (格式: "1.0.6")
      let versionStart = nameEnd + 1
      // 跳过可能的分隔符
      while (versionStart < response.length && (response[versionStart] === 0xAA || response[versionStart] === 0x00)) {
        versionStart++
      }

      // 提取版本号 (数字和点)
      let versionEnd = versionStart
      while (versionEnd < response.length &&
             ((response[versionEnd] >= 0x30 && response[versionEnd] <= 0x39) || // 数字
              response[versionEnd] === 0x2E)) { // 点
        versionEnd++
      }
      const firmwareVersion = String.fromCharCode(...Array.from(response.slice(versionStart, versionEnd))) || '1.0.6'
